import { ExternalLink, Pencil, Trash2, Phone, Star } from 'lucide-react';
    ...
    <div className={`bg-card rounded-2xl border shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group overflow-hidden ${doc.important ? 'border-copper/40 ring-2 ring-copper/15' : 'border-border'}`}>
      ...
      <div className="p-5 pb-3">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-text-light mt-1.5 font-medium">
      ...
      <div className="px-5 pb-5 space-y-3.5">
        ...
        <div className="flex flex-wrap gap-1.5">
          <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-semibold border ${STATUS_COLORS[doc.status] || 'border-border text-text-muted'}`}>
            {doc.status}
          </span>
          {(doc.tags || []).map(tag => (
            <span key={tag} className="px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-primary/5 text-primary/70">
              #{tag}
            </span>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2 pt-2 border-t border-border/40">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-bold rounded-xl bg-gradient-to-r from-copper to-gold-light text-white hover:from-gold-light hover:to-copper transition-all duration-200 shadow-sm hover:shadow-md hover:-translate-y-0.5"
          >
