...
export default function Header({ onExport, onImport, onAdd, onShowLanding }) {
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <div className="bg-gradient-to-r from-[#111119] via-[#1c1c2b] to-[#2a2a3d] backdrop-blur-sm text-white shadow-xl shadow-black/30 header-bar pt-[env(safe-area-inset-top)] max-lg:landscape:shadow-md">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-3 sm:py-4 max-lg:landscape:py-2 max-lg:landscape:gap-2 flex flex-col gap-3 sm:gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2.5 justify-end w-full md:w-auto items-center max-lg:landscape:gap-1 max-lg:landscape:flex-nowrap max-lg:landscape:overflow-x-auto max-lg:landscape:pb-0.5 max-lg:landscape:-mr-1 scrollbar-thin">
            ...
            <button
              type="button"
              onClick={onExport}
              className="header-btn-secondary inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 max-lg:landscape:px-2 max-lg:landscape:py-1.5 text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl transition-all duration-200 cursor-pointer !text-white shrink-0"
            >
              <Download className="w-4 h-4 shrink-0 text-white" />
              <span className="max-lg:landscape:hidden">Exporter</span>
            </button>

            <label className="header-btn-secondary inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 max-lg:landscape:px-2 max-lg:landscape:py-1.5 text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl transition-all duration-200 cursor-pointer !text-white shrink-0">
            ...
            <button
              type="button"
              onClick={onAdd}
              className="inline-flex flex-1 sm:flex-initial items-center justify-center gap-1.5 min-h-[40px] max-lg:landscape:min-h-[36px] sm:min-h-[44px] px-3 sm:px-5 py-2 max-lg:landscape:px-2.5 text-xs sm:text-sm font-bold rounded-lg sm:rounded-xl bg-gradient-to-r from-copper to-gold-light text-white hover:from-gold-light hover:to-copper transition-all duration-200 cursor-pointer shadow-lg shadow-copper/30 hover:shadow-copper/50 active:scale-[0.98] touch-manipulation shrink-0"
            >
